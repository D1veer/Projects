...
class Procedure:
  def __init__(self, staff: int, member: int, reason: str, procedure: str, duration: str, id: int, attachments: list):
...
class ProcedureManager:
  def __init__(self):
    ...
    with open(PROCEDUREDATABASE, "r") as f:
      data = json.load(f)
    for i in data:
      self.procedures.append(Procedure(data[i]["staff"], data[i]["member"], data[i]["reason"], data[i]["procedure"], data[i]["duration"], data[i]["id"], data[i]["attachments"]))
