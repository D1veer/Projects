...
class procedure(commands.Cog):
  ...
  async def procedure_def_add(self, ctx: commands.Context, member, reason, procedure, duration):
    ...
    procedure = Procedure.Procedure(ctx.author.id, acmember.id, reason, procedure, duration, procedureMananger.get_procedure_count(), attamnts)
  ...
  async def procedure_def_list(self, ctx: commands.Context, member: discord.Member = None):
    if member is not None:
      ...
      procedures = procedureMananger.get_procedures_by_member(ctx.author.id)
      ...
      for procedure in procedures:
        if v != 10:
          if (len(procedures) - 1) == v:
            ...
            📝 Reason : `{procedure.get_reason()}`
            ──────────────────────"""
          time = procedure.get_time().strftime("%Y/%m/%d %I:%M:%S %p")
  ...
  async def procedure_def_top_proofs(self, ctx: commands.Context):
    ...
    top_proofs.sort(key = lambda x: x[1], reverse = True)
    user_rank = []
    user_name = []
    user_xp = []
