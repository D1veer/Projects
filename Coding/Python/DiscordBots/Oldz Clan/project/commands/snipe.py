...
class snipe(commands.Cog):
	...
	async def on_message_delete(self, message):
		...
		if len(sniped_messages[message.channel.id]) == 100:
			del sniped_messages[message.channel.id][-1]
	...
	async def snipe(self, ctx, *, page: int = 1):
		...
		if role in ctx.author.roles:
			...
			embed.set_footer(text=f"{SERVER_NAME} | {page} / {len(sniped_messages[channel.id])}", icon_url=f"{SERVER_ICON}")
			await ctx.send(embed=embed)
		else:
			await ctx.send("__You do not have the required permissions to use this command.__")
